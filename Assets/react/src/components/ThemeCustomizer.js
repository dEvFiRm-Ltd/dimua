...
const ThemeCustomizer = (props) => {
  const [isHorizontalLayout, setHorizontalLayout] = useState(false);
  const [isBoxed, setBoxed] = useState(false);
  const [isSidebarScrollable, setSidebarScrollable] = useState(false);
  ...
  function _loadStateFromProps() {
    setHorizontalLayout(
      this.props.layoutType === layoutConstants.LAYOUT_HORIZONTAL
    );
    setBoxed(this.props.layoutWidth === layoutConstants.LAYOUT_WIDTH_BOXED);
    setSidebarScrollable(
      this.props.leftSideBarType ===
        layoutConstants.LEFT_SIDEBAR_TYPE_SCROLLABLE
    );
    setCondensed(
      this.props.leftSideBarType === layoutConstants.LEFT_SIDEBAR_TYPE_CONDENSED
    );
    setDark(
